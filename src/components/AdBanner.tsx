    ...
    <>
          ...
          <div className="relative bg-gradient-to-b from-card/98 via-card/95 to-card backdrop-blur-3xl">
            ...
            <a href={banner.link_url} target="_blank" rel="noopener noreferrer" className="block group">
              {banner.image_url ? (
                <div className="relative overflow-hidden">
                  <img src={banner.image_url} alt={banner.title} className="w-full h-44 object-cover transition-all duration-1000 group-hover:scale-110 group-hover:brightness-110" />
              ...
              <div className="relative p-5 space-y-3">
                <h4 className="text-base font-bold text-foreground group-hover:text-accent transition-colors duration-300 leading-snug line-clamp-2">{banner.title}</h4>
                {banner.description && <p className="text-sm text-muted-foreground/70 line-clamp-3 leading-relaxed">{banner.description}</p>}
                <div className="pt-1">
            ...
            {banners.length > 1 && (
              <div className="flex justify-center gap-2 pb-4">
                {banners.map((_, i) => (
                  <button key={i} onClick={() => setCurrentIndex(i)} className={`rounded-full transition-all duration-500 ${i === currentIndex ? "w-6 h-2 bg-accent shadow-sm shadow-accent/50" : "w-2 h-2 bg-muted-foreground/20 hover:bg-muted-foreground/40"}`} />
                ))}
              </div>
            )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 lg:hidden animate-fade-in">
        <div className="relative overflow-hidden border-t border-accent/15 bg-gradient-to-r from-card/98 via-card/95 to-card backdrop-blur-2xl shadow-[0_-4px_30px_-8px_hsl(var(--accent)/0.2)]">
          ...
          <button
            onClick={() => setDismissed(true)}
            className="absolute top-2 right-2 z-20 p-1.5 rounded-lg bg-background/50 backdrop-blur-md text-muted-foreground hover:text-foreground transition-all duration-200"
            aria-label="Закрыть"
          >
            <X className="h-3 w-3" />
          </button>

          <a href={banner.link_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 pr-10 group">
            {banner.image_url ? (
              <img src={banner.image_url} alt={banner.title} className="w-16 h-16 rounded-xl object-cover flex-shrink-0 ring-1 ring-accent/10" />
            ) : (
              <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-accent/10 to-primary/5 flex items-center justify-center flex-shrink-0">
                <Sparkles className="h-5 w-5 text-accent/40" />
              </div>
            )}
            <div className="flex-1 min-w-0 space-y-1">
              <h4 className="text-sm font-bold text-foreground leading-tight line-clamp-1">{banner.title}</h4>
              {banner.description && <p className="text-xs text-muted-foreground/60 line-clamp-1">{banner.description}</p>}
          ...
          {banners.length > 1 && (
            <div className="flex justify-center gap-1.5 pb-2 -mt-1">
              {banners.map((_, i) => (
                <button key={i} onClick={() => setCurrentIndex(i)} className={`rounded-full transition-all duration-400 ${i === currentIndex ? "w-5 h-1.5 bg-accent" : "w-1.5 h-1.5 bg-muted-foreground/20"}`} />
              ))}
            </div>
          )}
